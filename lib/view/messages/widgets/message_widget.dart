...
class MessageDesign extends StatelessWidget {
   MessageDesign({
    ...
    required this.controller, required this.roomModel,
  });
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      ...
      if(snapshot.hasData && snapshot.connectionState==ConnectionState.done){
        final LatestMessageModel latestModel= snapshot.data as LatestMessageModel;
        return InkWell(
          onTap: () {
            Get.to(
                  () =>  ChatView(),
                            ...
                            children: [
                              Expanded(
                                child: Text('Deep Cleaning ',
                                    style: TextStyle(
                                        color: ColorApp.primary, fontSize: 14)),
                              ),
